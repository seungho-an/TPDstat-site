...
Use `dplyr` functions to create a table with the number of candidates in each type of office in the data set. Use the function `knitr::kable()` on this table to have a nicely formatted table produced in the knitted output. 
...
In the write up, describe which type of candidate sponsored the most attack ads on average. 
...
You should save the `ggplot` output as `fb_top_plot`. 
